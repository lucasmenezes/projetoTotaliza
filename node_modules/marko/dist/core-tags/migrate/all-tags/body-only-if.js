...
module.exports = function migrator(node, context) {
    ...
    if (arg) {
        ...
        const tagName = node.rawTagNameExpression || (!tagDef.html ? importTag(tagDef.renderer || tagDef.template, context) : builder.literal(node.tagName));

        dynamicTag.rawTagNameExpression = printJS(builder.conditionalExpression(builder.expression(arg.argument), builder.literalNull(), tagName), context);
